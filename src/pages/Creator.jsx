...
export default function Creator() {
  ...
  // Fetch only this creator's events by email
  const fetchEvents = async (email) => {
    try {
      const res = await fetch(
        `http://localhost:5000/api/events/creator/${email}`
      );
  ...
  const handleSubmit = async (e) => {
    ...
    if (res.ok) {
      ...
      setShowForm(false);
      fetchEvents(user.email); // always fetch after create
    } else {
  ...
  const handleDelete = async (id) => {
    ...
    if (res.ok) {
      ...
      fetchEvents(user.email); // always fetch after delete
    }
  };
  ...
  const handleEditSubmit = async (e) => {
    ...
    const res = await fetch(`http://localhost:5000/api/events/${editEvent._id}`, {
      ...
      body: JSON.stringify({
        ...
        location: editEvent.location || editEvent.city,
        registrationUrl: editEvent.registrationUrl,
        interest: editEvent.interest,
      }),
    });
    const data = await res.json();
    if (res.ok) {
      ...
      setEditEvent(null);
      fetchEvents(user.email); // always fetch after update
    } else {
      setMessage("❌ Error: " + data.error);
    }
  };
      ...
      <header className="bg-[#1e1e2f] text-white px-6 py-4 flex justify-between items-center">
        ...
        <div className="flex gap-4 items-center">
          <button
            ...
            className="bg-gray-700 px-3 py-2 rounded-lg font-semibold hover:bg-gray-900"
          >
            Logout
          </button>
          <button
          ...
          <div className="bg-white p-6 rounded-xl shadow-lg mb-8 max-w-md mx-auto">
            ...
            <form onSubmit={handleSubmit} className="space-y-4">
              ...
              <input
                ...
                placeholder="Registration Website URL"
        ...
        <div className="flex flex-wrap justify-center gap-6">
          {events.length === 0 ? (
            ...
            events.map((event) => (
              <div
                key={event._id}
                className="bg-white rounded-lg shadow-md w-72 overflow-hidden transform hover:scale-105 transition"
              >
                <img
                  src={event.imageUrl}
                  alt={event.title}
            ...
            <div className="bg-white rounded-lg shadow-lg p-8 max-w-md w-full relative">
              ...
              <form onSubmit={handleEditSubmit} className="space-y-4">
                ...
                <input
                  type="url"
                  name="registrationUrl"
                  value={editEvent.registrationUrl}
                  onChange={handleEditChange}
                  className="w-full border rounded-lg px-3 py-2"
                />
                <select
                  name="interest"
                  value={editEvent.interest}
                  onChange={handleEditChange}
                  className="w-full border rounded-lg px-3 py-2"
                  required
                >
